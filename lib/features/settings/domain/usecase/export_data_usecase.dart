...
class ExportDataUsecase {
  final _log = Logger('ExportDataUsecase');
  ...
  Future<String> exportFoodData(DateTime startDate, DateTime endDate) async {
    try {
      _log.fine('Starting food data export from $startDate to $endDate');

      if (startDate.isAfter(endDate)) {
        _log.severe('Invalid date range: start date after end date');
      ...
      for (DateTime date = startDate;
        ...
        try {
          _log.fine('Fetching intakes for date: $date');
          final breakfast =
              await _getIntakeUsecase.getBreakfastIntakeByDay(date);
          ...
          _log.severe('Error fetching intakes for date $date: $e\n$stackTrace');
          Sentry.captureException(e, stackTrace: stackTrace);
        }
      }

      _log.fine('Writing ${allIntakes.length} intakes to CSV');
      ...
      final fileName =
          'food_diary_${DateTime.now().millisecondsSinceEpoch}.csv';
      final file = File('${directory.path}/$fileName');

      try {
        await file.writeAsString(csv.toString(), encoding: utf8);
        _log.fine('Successfully exported data to: ${file.path}');
        return file.path;
      } catch (e, stackTrace) {
        _log.severe('Failed to write export file: $e\n$stackTrace');
        Sentry.captureException(e, stackTrace: stackTrace);
        throw Exception('Failed to write export file: $e');
      }
    } catch (e, stackTrace) {
      _log.severe('Export operation failed: $e\n$stackTrace');
      Sentry.captureException(e, stackTrace: stackTrace);
